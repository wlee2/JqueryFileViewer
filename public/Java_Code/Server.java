...
class Data {
	...
	public void mapToArray() {
		...
		System.out.println( "Map to ArrayList Time : " + ( end - start )/1000.0 );
	}
	...
	public void FinalDivider() {
		...
		System.out.println("\t-(Message)The server starts the conversion for output. It can take a long time.");
		...
		System.out.println("\n\t-(Message)The output has been successfully saved.");
		long end = System.currentTimeMillis();
		System.out.println( "Output Makeing Time : " + ( end - start )/1000.0 );
	}
}
...
class CreateServer extends Thread {
	...
	public void run() {
		try {			
			...
			System.out.println("Connection from Socket[address=" + socket.getInetAddress() + "/" + socket.getPort() + "/" + socket.getLocalPort() + "]");						
			...
			System.out.println("Connection from Socket[address=" + socket.getInetAddress() + "/" + socket.getPort() + "/" + socket.getLocalPort() + "] == logout");
			pw.close();
			br.close();
			socket.close();
		} catch (Exception e) {
